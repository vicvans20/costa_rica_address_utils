...
To release a new version, update the version number in `version.rb` and `Gemfile.lock`, and then run `bundle exec rake release`, which will create a git tag for the version, push git commits and the created tag, and push the `.gem` file to [rubygems.org](https://rubygems.org).

> `Gemfile.lock` update for version should be applied on `costa_rica_address_utils (x.x.x)` line
